...
describe('read_file tool', () => {
  ...
  it('withholds a file with a secret and never leaks it', async () => {
...
describe('list_files tool', () => {
  it('omits denied files and marks nothing when clean', async () => {
  ...
  it('reports when nothing can be shared', async () => {
    const res = await server({ '.env': `OPENAI_API_KEY=${OPENAI}\n` }).handle({
      jsonrpc: '2.0',
      id: 1,
      method: 'tools/call',
